...
SERVERS_IP = [ip.strip() for ip in os.getenv("SERVERS_IP", "").split(',') if ip.strip()]
SERVERS_URL = [url.strip() for url in os.getenv("SERVERS_URL", "").split(',') if url.strip()]
DATABASE = os.getenv("DATABASE_PATH")
USERSDATABASE = os.getenv("USERSDATABASE_PATH")
ADD_CLIENT_URL = os.getenv("ADD_CLIENT_URL")
ADD_CLIENT_URL_LIST = [u.strip() for u in os.getenv("ADD_CLIENT_URL_LIST", "").split(',') if u.strip()]
ID_1 = os.getenv("ID_1")
ID_2 = os.getenv("ID_2")
ID_3 = os.getenv("ID_3")
...
def get_server_for_new_user():
    for i in range(len(SERVERS_IP)):
        if server_usage[i] < 300:
            return (SERVERS_IP[i], SERVERS_URL[i], i)

    return (SERVERS_IP[0], SERVERS_URL[0], 0)
...
async def initialize_database():
    async with aiosqlite.connect(USERSDATABASE) as db:
        ...
        # Создаем таблицу pending_payments
        await db.execute("""
        ...
        column_names = [column[1] for column in columns]  # Добавьте эту строку
        ...
        await db.commit()
        bot_logger.info("База данных инициализирована успешно")
            ...
            try:
                async with session.get(url, ssl=False) as response:
                    if response.status == 200:
                        results.append(f"{url} ✅ СЕРВЕР АКТИВЕН")
                    else:
                        results.append(f"{url} ⛔️ СЕРВЕР НЕДОСТУПЕН (status {response.status})")
            except aiohttp.ClientError as e:
                results.append(f"{url} ⚠️ ОШИБКА СОЕДИНЕНИЯ: {e}")
    return "\n".join(results)
...
async def show_instructions(message: types.Message):
    ...
    instructions = [
        ("📱 iOS", "https://vc.ru/u/4353552-shahid-zhabrailov/1724917"),
        ("🤖 Android", "https://vc.ru/u/4353552-shahid-zhabrailov/1724867"),
...
async def confirm_purchase(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        ...
        "⚠️ После оплаты нужно подождать ±1 минуту для обработки платежа",
...
async def cancel_purchase(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        ...
        "❌ Вы отменили создание нового подключения.",
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🔄 Начать заново", callback_data="buy_vpn")],
...
async def add_client(message, name, expiry_time, id_vless, telegram_id):
    try:
        server_ip, server_url, server_idx = get_server_for_new_user()
        session_id = await login()
        if session_id:
            if name is not None and expiry_time is not None:
                client_id = ''.join(random.choices(string.ascii_letters + string.digits, k=20))
                success = await add_client_request(message, session_id, name, expiry_time, client_id, id_vless, server_ip, server_url, server_idx)
                if success:
                    try:
...
async def add_client_request(message, session_id, name, expiry_time, client_id, id_vless, server_ip, server_url, server_idx):
    ...
    full_add_url = ADD_CLIENT_URL_LIST[server_idx]

    bot_logger.info(f"Отправляемые данные клиента: {json.dumps(client_data, ensure_ascii=False, indent=2)}")
    bot_logger.info(f"Запрос на URL: {full_add_url}")

    async with aiohttp.ClientSession() as session:
        async with session.post(full_add_url, headers=headers, json=client_data, ssl=False) as response:
            ...
            if response.status == 200:
                ...
                if response_json.get("success"):
                    server_usage[server_idx] += 1
                    bot_logger.info(f"Сервер {server_ip} теперь используется {server_usage[server_idx]} пользователями")
                    if message:
                        await message.answer(
                            "🎉 <b>Поздравляем!</b>\n\n"
...
async def generate_config(client_id, email, prt=None, pot=None, pbk=None, fp=None, sni=None, sid=None, net=None, secur=None):
    ...
    config_str = f"{prt}://{client_id}@{SERVER_IP}:{pot}?type={net}&security={secur}&pbk={pbk}&fp={fp}&sni={sni}&sid={sid}&spx=%2F&flow=xtls-rprx-vision#{email}"
...
def get_total_gb(value):
    gb_value = os.getenv(f'TOTAL_GB_{value}')
    if not gb_value:
        return UNLIMITED_TRAFFIC
    if gb_value.lower() == 'unlimited':
        return UNLIMITED_TRAFFIC
    try:
        return int(gb_value)
...
async def check_subscription_expiry():
    ...
    bot_logger.info(f"Проверка подписок. Текущее время: {current_time}")

    try:
        async with aiosqlite.connect(USERSDATABASE) as db:
            # Получаем всех пользователей с их email
            cursor = await db.execute("SELECT telegram_id, email FROM users WHERE email IS NOT NULL")
            users = await cursor.fetchall()

            for user in users:
                telegram_id, emails = user
                if not emails:
                    continue

                # Проверяем каждый email пользователя
                for email in emails.split(','):
                    async with aiosqlite.connect(DATABASE) as db_traffic:
                        cursor = await db_traffic.execute(
                            "SELECT expiry_time FROM client_traffics WHERE email = ?", 
                            (email,)
                        )
                        result = await cursor.fetchone()
                        
                        if result:
                            expiry_timestamp = result[0]
                            # Конвертируем timestamp в datetime
                            expiry_date = datetime.datetime.fromtimestamp(abs(expiry_timestamp) / 1000)
                            
                            # Вычисляем разницу в днях
                            days_left = (expiry_date - current_time).days
                            
                            bot_logger.info(
                                f"Проверка для ID {telegram_id}, email {email}: "
                                f"дата окончания {expiry_date}, осталось дней: {days_left}"
                            )

                            # Отправляем уведомление если осталось 7, 3, 2, 1 день или сегодня последний день
                            if days_left in [7, 3, 2, 1, 0]:
                                await send_expiry_notification(telegram_id, days_left)

    except Exception as e:
        bot_logger.error(f"Ошибка при проверке подписок: {e}")
...
async def send_expiry_notification(telegram_id, days_left):
    try:
        if days_left == 0:
            notification_text = (
                "⚠️ <b>ВНИМАНИЕ!</b>\n\n"
                "Сегодня последний день действия вашей VPN-подписки!\n\n"
                "🔄 Чтобы продолжить пользоваться сервисом без перерывов, "
                "пожалуйста, продлите подписку прямо сейчас."
            )
        elif days_left == 1:
            notification_text = (
                "⚠️ <b>ВНИМАНИЕ!</b>\n\n"
                ...
                "чтобы избежать перерыва в работе сервиса."
            )
        else:
            days_word = get_days_word(days_left)
            notification_text = (
                "ℹ️ <b>Напоминание</b>\n\n"
                f"До окончания срока действия вашей VPN-подписки осталось {days_left} {days_word}.\n\n"
                "🔄 Не забудьте продлить подписку, чтобы продолжить пользоваться сервисом."
            )

        # Добавляем кнопку продления
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton("💳 Продлить подписку", callback_data="buy_vpn"))
        ...
        bot_logger.info(f"Уведомление успешно отправлено пользователю {telegram_id}")
        
    except Exception as e:
        bot_logger.error(f"Ошибка при отправке уведомления пользователю {telegram_id}: {e}")
...
async def create_yookassa_payment(amount, user_id, name, expiry_time):
    ...
    try:
        payment = Payment.create({
            ...
            "capture": True,
            "description": f"Оплата VPN подписки для {name}",
            "metadata": {
                "user_id": str(user_id),
                "name": name,
        ...
        async with aiosqlite.connect(USERSDATABASE) as db:
            await db.execute("""
                INSERT INTO pending_payments (payment_id, user_id, name, expiry_time, amount, status)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (payment.id, user_id, name, expiry_time, amount, 'pending'))
            await db.commit()
        
        bot_logger.info(f"Создан платеж {payment.id} для пользователя {user_id}")
        return payment
        
    except Exception as e:
        bot_logger.error(f"Ошибка при создании платежа: {str(e)}")
        raise
        ...
        try:
            async with aiosqlite.connect(USERSDATABASE) as db:
                async with db.execute("""
                    SELECT payment_id, user_id, name, expiry_time, amount
                    FROM pending_payments
                    WHERE status = 'pending'
                """) as cursor:
                    pending_payments = await cursor.fetchall()

                for payment_id, user_id, name, expiry_time, amount in pending_payments:
                    payment = await check_yookassa_payment(payment_id)
                    
                    if payment and payment.status == "succeeded":
                        # Создаем VPN подключение
                        message = types.Message(chat={'id': user_id})
                        await add_client(message, name, expiry_time, ID_2, user_id)
            ...
            bot_logger.error(f"Error in check_pending_payments: {e}")
...
async def check_yookassa_payment(payment_id):
    try:
        ...
        bot_logger.info(f"Проверка платежа {payment_id}: статус {payment.status}")
        return payment
    except Exception as e:
        bot_logger.error(f"Ошибка при проверке платежа {payment_id}: {e}")
        return None
...
async def cancel_payment(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await callback_query.message.edit_text(
            "❌ <b>Оплата отменена</b>\n\n"
            "Вы можете начать процесс покупки заново или вернуться в главное меню.",
            parse_mode=ParseMode.HTML,
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="🔄 Начать заново", callback_data="buy_vpn")],
                [types.InlineKeyboardButton(text="🏠 Главное меню", callback_data="back_to_main")]
        ...
        bot_logger.error(f"Ошибка при отмене платежа: {e}")
